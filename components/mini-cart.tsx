    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Cart panel — slides in from the left for RTL */}
          <motion.div
            initial={{ x: "-100%" }}
            animate={{ x: 0 }}
            exit={{ x: "-100%" }}
            transition={{ type: "tween", duration: 0.4, ease: "easeInOut" }}
            className="fixed left-0 top-0 bottom-0 w-full max-w-md bg-background z-50 flex flex-col"
          >
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-border">
              <h2 className="font-serif text-xl">سبد خرید</h2>
              <button
                onClick={onClose}
            ...
            <div className="border-t border-border p-6 space-y-4">
              ...
              <button
                onClick={onClose}
                className="w-full text-center text-sm underline underline-offset-4 hover:no-underline transition-all"
              >
                ادامه خرید
              </button>
            </div>
